import React, {  useState } from 'react'

import { FontAwesomeIcon } from '@fortawesome/react-fontawesome'
import { faChevronDown } from '@fortawesome/free-solid-svg-icons'
import { faPlane, faLocationDot } from '@fortawesome/free-solid-svg-icons'
import {anticipate, motion } from 'motion/react'
const Home = () => {
  const [pickup, setpickup] = useState('')
  const [destination, setdestination] = useState('')
  const [isClicked, setIsClicked] = useState(false);
  return (
    <div className='h-screen w-screen '>
        <img
        className="w-24  absolute left-6 top-6"
        src="data:image/png;base64,"
        alt=""
      />
      <img  className='h-full w-full object-cover' src="https://images.squarespace-cdn.com/content/v1/54ff63f0e4b0bafce6932642/1613584766993-KD4G7Q9XDVVHE7EFE1JF/Two+Maps+-+Grayscale.png" alt="" />
      
      <motion.div 
     initial={{ bottom: 0, top: "auto" }} // Start at bottom: 0
     animate={isClicked ? { top: 0,bottom: "auto" } : { bottom: 0, top: "auto" }} // Move to top: 0 when clicked
     transition={{ type: "spring", stiffness:  80 }}
     
      className="absolute bottom-0 w-full bg-white p-5">
     <div className='flex justify-between items-center'>
      <h2 className='text-3xl '>Find a trip</h2>
      <FontAwesomeIcon 
      onClick={(e) => { setIsClicked(false)
        console.log(isClicked, e.target);
      }
      }
      className='w-4 cursor-pointer'
      icon={faChevronDown} />
     </div>
      <form 
      onClick={() => setIsClicked(true)}
       onSubmit={(e)=>{
        submitHandaler(e)
       }}
      action="" className="mt-2  flex flex-col ">
       
    
       <input 
       required
       value={pickup}
       onChange={(e)=>{
        setpickup(e.target.value)
       }}
       type="text"
        placeholder=" Your Pick up location" 
       className="w-full mt-2  bg-gray-100 text-lg p-2.5 rounded outline-none"
       />
        
       <input
       required
        value={destination}
        onChange={(e)=>{
         setdestination(e.target.value)
        }}
        type="text" 
        placeholder="Type your DESTINATION" 
        className=" mt-2  bg-gray-100 text-lg p-2.5 rounded outline-none"
       />
      </form>
      {/* location shower */}
      <motion.div
      initial={{display: "none", opacity: 0}}
      animate={isClicked ? {display: "flex", opacity: 1} : {display: "none", opacity: 0}}
       className=' h-screen w-screen mt-7 flex flex-col gap-2'>
     <div className='flex justify-start items-center gap-2.5'>
     <FontAwesomeIcon className='w-4 bg-gray-200 p-2 rounded-full' icon={faPlane} />
     <div className='ml-0.5 p-0.5'>
        <h1 className='text-normal font-bold'>Lorem ipsum dolor sit amet.</h1>
        <h3 className='text-sm font-normal text-zinc-600'>Lorem ipsum dolor sit.</h3>
     </div>
     </div>
     <div className='flex justify-start items-center gap-2.5'>
     <FontAwesomeIcon className='w-4 bg-gray-200 p-2 rounded-full' icon={faLocationDot} />
     <div className='ml-0.5 p-0.5'>
        <h1 className='text-normal font-bold'>Lorem ipsum dolor sit amet.</h1>
        <h3 className='text-sm font-normal text-zinc-600'>Lorem ipsum dolor sit.</h3>
     </div>
     </div> 




     <div className='flex justify-start items-center gap-2.5'>
     <FontAwesomeIcon className='w-4 bg-gray-200 p-2 rounded-full' icon={faPlane} />
     <div className='ml-0.5 p-0.5'>
        <h1 className='text-normal font-bold'>Lorem ipsum dolor sit amet.</h1>
        <h3 className='text-sm font-normal text-zinc-600'>Lorem ipsum dolor sit.</h3>
     </div>
     </div>
     <div className='flex justify-start items-center gap-2.5'>
     <FontAwesomeIcon className='w-4 bg-gray-200 p-2 rounded-full' icon={faLocationDot} />
     <div className='ml-0.5 p-0.5'>
        <h1 className='text-normal font-bold'>Lorem ipsum dolor sit amet.</h1>
        <h3 className='text-sm font-normal text-zinc-600'>Lorem ipsum dolor sit.</h3>
     </div>
     </div> 



     <div className='flex justify-start items-center gap-2.5'>
     <FontAwesomeIcon className='w-4 bg-gray-200 p-2 rounded-full' icon={faPlane} />
     <div className='ml-0.5 p-0.5'>
        <h1 className='text-normal font-bold'>Lorem ipsum dolor sit amet.</h1>
        <h3 className='text-sm font-normal text-zinc-600'>Lorem ipsum dolor sit.</h3>
     </div>
     </div>
     <div className='flex justify-start items-center gap-2.5'>
     <FontAwesomeIcon className='w-4 bg-gray-200 p-2 rounded-full' icon={faLocationDot} />
     <div className='ml-0.5 p-0.5'>
        <h1 className='text-normal font-bold'>Lorem ipsum dolor sit amet.</h1>
        <h3 className='text-sm font-normal text-zinc-600'>Lorem ipsum dolor sit.</h3>
     </div>
     </div> 






     
     
    </motion.div>
       </motion.div>
    
    </div>
  )
}

export default Home