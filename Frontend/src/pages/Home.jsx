import React, { useState } from "react";
import Confirmvehicle from "../components/Confirmvehicle";
import { FontAwesomeIcon } from "@fortawesome/react-fontawesome";
import { faChevronDown } from "@fortawesome/free-solid-svg-icons";
import { faPlane, faLocationDot } from "@fortawesome/free-solid-svg-icons";
import { motion } from "motion/react";
import Showvehicle from "../components/Showvehicle";
import LookingforDriver from "../components/LookingforDriver";
const Home = () => {
  const locations = [
    { icon: faPlane, title: "JFK Airport", subtitle: "New York, NY" },
    { icon: faLocationDot, title: "Central Park", subtitle: "New York, NY" },
    { icon: faPlane, title: "LAX Airport", subtitle: "Los Angeles, CA" },
    {
      icon: faLocationDot,
      title: "Hollywood Sign",
      subtitle: "Los Angeles, CA",
    },
    { icon: faPlane, title: "O'Hare Airport", subtitle: "Chicago, IL" },
    { icon: faLocationDot, title: "Millennium Park", subtitle: "Chicago, IL" },
  ];
  const [pickup, setpickup] = useState("");
  const [destination, setdestination] = useState("");
  const [isClicked, setIsClicked] = useState(false);
  const [openvehiclepanel, setopenvehiclepanel] = useState(false);
  const [openvehicleconfirm, setopenvehicleconfirm] = useState(false);
  const [lookingDriver, setlookingDriver] = useState(false)
  return (
    <div className="h-screen w-screen ">
      <img
        className="w-24  absolute left-6 top-6"
        src="data:image/png;base64,"
        alt=""
      />
      <img
        className="h-full w-full object-cover"
        src="https://images.squarespace-cdn.com/content/v1/54ff63f0e4b0bafce6932642/1613584766993-KD4G7Q9XDVVHE7EFE1JF/Two+Maps+-+Grayscale.png"
        alt=""
      />

      <motion.div
        initial={{ bottom: 0, top: "auto" }} // Start at bottom: 0
        animate={
          isClicked ? { top: 0, bottom: "auto" } : { bottom: 0, top: "auto" }
        } // Move to top: 0 when clicked
        transition={{ type: "spring", stiffness: 80 }}
        className="absolute bottom-0 w-full bg-white p-5"
      >
        <div className="flex justify-between items-center">
          <h2 className="text-3xl ">Find a trip</h2>
          <FontAwesomeIcon
            onClick={(e) => {
              setIsClicked(false);
            }}
            className="w-4 cursor-pointer"
            icon={faChevronDown}
          />
        </div>
        <form
          onClick={() => setIsClicked(true)}
          onSubmit={(e) => {
            submitHandaler(e);
          }}
          action=""
          className="mt-2  flex flex-col "
        >
          <input
            required
            value={pickup}
            onChange={(e) => {
              setpickup(e.target.value);
            }}
            type="text"
            placeholder=" Your Pick up location"
            className="w-full mt-2  bg-gray-100 text-lg p-2.5 rounded outline-none"
          />

          <input
            required
            value={destination}
            onChange={(e) => {
              setdestination(e.target.value);
            }}
            type="text"
            placeholder="Type your DESTINATION"
            className=" mt-2  bg-gray-100 text-lg p-2.5 rounded outline-none"
          />
        </form>
        {/* location shower */}
        <motion.div
          initial={{ display: "none", opacity: 0 }}
          animate={
            isClicked
              ? { display: "flex", opacity: 1 }
              : { display: "none", opacity: 0 }
          }
          className=" h-screen w-screen mt-7 flex flex-col gap-2"
        >
          {locations.map((location, index) => (
            <div
              onClick={() => {
                setopenvehiclepanel(true);
                setIsClicked(false);
              }}
              key={index}
              className="flex justify-start items-center mr-2 gap-2.5 active:bg-gray-100 p-2.5 active:border-2 active:border-zinc-400 rounded cursor-pointer"
            >
              <FontAwesomeIcon
                className="w-4 bg-gray-200 p-2 rounded-full"
                icon={location.icon}
              />
              <div className="ml-0.5 p-0.5">
                <h1 className="text-normal font-bold">{location.title}</h1>
                <h3 className="text-sm font-normal text-zinc-600">
                  {location.subtitle}
                </h3>
              </div>
            </div>
          ))}
        </motion.div>
      </motion.div>

      <Showvehicle
        setIsClicked={setIsClicked}
        setopenvehiclepanel={setopenvehiclepanel}
        openvehiclepanel={openvehiclepanel}
        setopenvehicleconfirm={setopenvehicleconfirm}
      />
      <Confirmvehicle
        openvehicleconfirm={openvehicleconfirm}
        setopenvehicleconfirm={setopenvehicleconfirm}
        setIsClicked={setIsClicked}
        setopenvehiclepanel={setopenvehiclepanel}
        setlookingDriver={setlookingDriver}
        locations={locations}
      />
    <LookingforDriver
     openvehicleconfirm={openvehicleconfirm}
     setopenvehicleconfirm={setopenvehicleconfirm}
     setIsClicked={setIsClicked}
     lookingDriver={lookingDriver}
     setlookingDriver={setlookingDriver}
     locations={locations}
    />
    </div>
  );
};

export default Home;
